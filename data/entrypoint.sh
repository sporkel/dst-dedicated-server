...
if [ ! "${SHARD_NAME,,}" = "master" ]; then
    ...
    # Without this, a secondary shard generates as a second Forest instead of an
    # actual cave (all sinkholes come up disabled and migration fails)
    if [ ! -f "/data/$CLUSTER_NAME/$SHARD_NAME/leveldataoverride.lua" ]; then
        cp "$HOME/data/leveldataoverride_caves.lua" "/data/$CLUSTER_NAME/$SHARD_NAME/leveldataoverride.lua"
    fi
fi
